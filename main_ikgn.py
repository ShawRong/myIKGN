...
def train_network(network,file,args, criterion = None):
    ...
    for epoch in range(args['epochs']):
        ...
        for kg_train_batch in minibatch(kg_data, batch_size=args['batch_size']):
            batch_head, batch_relation, batch_pos_tail, batch_neg_tail = generate_kg_batch(train_kg_dict_inverse,kg_train_batch,n_loc_entity,device)
        ...
        relations=list(laplacian_dict.keys())
        network('update_att',h_list, t_list, r_list, relations,device)
        ...
        if epoch >15:
            ...
            if max_metric < metric[1]:
                ...
                '_'+str(params['hidden_size'])+'_' +str(params['layers'])+'_' +str(params['aggregator_type'])+'_model.pkl')
            else:
                worse_round += 1
            print('worse_round',worse_round)
...
def get_params():
    ...
    # Training settings
    parser = argparse.ArgumentParser(description='IKGN')
    parser.add_argument("--data_dir", type=str,
                        default="./data/",help="data directory")
    parser.add_argument("--data", type=str,
                        default="nyc_category",help="data name")
    parser.add_argument("--gpu", type=int,
                        default="0",help="choose gpu")
    parser.add_argument('--batch_size', type=int, default=128, metavar='N',
                        help='input batch size for training (default: 64)')
    parser.add_argument("--hidden_size", type=int, default=130, metavar='N',
                        help='hidden layer size')
    parser.add_argument("--user_poi_size", type=int, default=300, metavar='N',
                        help='hidden layer size')
...
if __name__ == '__main__':
    ...
    kg = data['KG']
    print(len(data['data_neural'][0]['sessions']))
    sum = 0
    for i in data['data_neural']:
        sum += len(data['data_neural'][i]['sessions'])
    data_neural = data['data_neural'] 
    poi_coordinate = data['vid_lookup']
    tim_gap = kg['tim_rel'] #time
    dis_gap = kg['dis_rel'] #distance
    ...
    n_users = uid_size
    n_locs = loc_size
    n_category=category_size
    n_tim_gap = len(tim_gap)
    n_dis_gap = len(dis_gap)
    n_tim_rel = len(tim_rel)
    print('user and loc and category num:',n_users,n_locs,n_category)
    session_id_sequences = None
    user_id_session = None

    h_list, r_list, t_list, laplacian_dict, A_in,train_kg_dict_inverse ,kg_data= generate_kg_h_r_t(kg['train_kg'],n_users+n_locs+n_category)
    relations = list(laplacian_dict.keys())

    network = IKGN(params,n_users=n_users, n_locs=n_locs,n_category=n_category,n_tim_rel=n_tim_rel, n_tim_dis_gap=ptp_size,\
    data_neural=data_neural, kg=kg,A_in=A_in).to(device)
    ...
    result_path = open('./result/'+str(ticks) +str(params['data']) +'_'+ str(params['batch_size']) +'_' +str(params['lr'])+\
               '_'+str(params['hidden_size'])+'_' +str(params['layers'])+'_' +str(params['aggregator_type'])+'.txt','a')
